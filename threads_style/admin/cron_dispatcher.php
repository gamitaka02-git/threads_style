 ...
 * 0 * * * * /usr/bin/php /path/to/admin/cron_dispatcher.php >> /dev/null 2>&1
 *
 * ============================================================
 */
...
// ヘルパー関数
function get_config_cron($key, $default = '') {
    global $pdo;
    try {
...
function set_config_cron($key, $value) {
    global $pdo;
    try {
...
// Threads API関数の読み込み
require_once __DIR__ . '/api/threads_api.php';
...
$auto_post_enabled = get_config_cron('auto_post_enabled', '0');
if ($auto_post_enabled !== '1') {
    cron_log("自動投稿は無効です。スキップします。");
} else {
    $variance = (int)get_config_cron('post_interval_variance', 30);

    // 現在時刻 ± ゆらぎ の範囲内に予約されている投稿を検索
    // ゆらぎ: 厳密な時刻ではなく、前後の幅を持たせてBot判定を回避
    ...
    if (empty($pending_posts)) {
        ...
        foreach ($singles as $post) {
            // ゆらぎ（ランダム遅延）
            if ($variance > 0) {
                $delay = rand(0, $variance * 60); // 秒に変換
                cron_log("ゆらぎ遅延: {$delay}秒（投稿ID: {$post['id']}）");
                sleep(min($delay, 60)); // 最大60秒まで（cron全体が長くなりすぎないよう）
            }

            $result = threads_publish_post($post['content'], $post['ai_label']);
            if ($result['success']) {
                $stmt = $pdo->prepare("
        ...
        foreach ($groups as $group_id => $thread_posts) {
            ...
            cron_log("スレッド投稿開始 グループID:{$group_id} ({$count}件)");
            $result = threads_publish_thread($contents);

            if ($result['success']) {
                foreach ($thread_posts as $i => $post) {
                    $tid = isset($result['results'][$i]) ? ($result['results'][$i]['post_id'] ?? '') : '';
                    $stmt = $pdo->prepare("
                        UPDATE posts SET status = 'posted', posted_at = datetime('now'), threads_post_id = :tid
                        WHERE id = :id
                    ");
                    $stmt->execute([':tid' => $tid, ':id' => $post['id']]);
                }
...
$token_expires = get_config_cron('threads_token_expires_at');
if (!empty($token_expires)) {
    $expires_ts = strtotime($token_expires);
    $days_left = ($expires_ts - time()) / 86400;

    if ($days_left <= 7 && $days_left > 0) {
        cron_log("トークン残り{$days_left}日 - 自動更新を実行します");
        $refresh_result = threads_refresh_token();

        $stmt = $pdo->prepare("INSERT INTO token_logs (action, status, message) VALUES ('auto_refresh', :status, :msg)");
        $stmt->execute([
            ':status' => $refresh_result['success'] ? 'success' : 'failed',
            ':msg'    => $refresh_result['message'] ?? '',
        ]);
...
$profile_result = threads_get_user_profile();
if ($profile_result['success'] && isset($profile_result['profile'])) {
    // Threads APIのプロフィールにはフォロワー数が含まれない場合がある
    // User Insights APIを使用
    $access_token = get_config_cron('threads_access_token');
    $user_id = get_config_cron('threads_user_id');
...
// 5. 最終実行時刻の記録
// ============================================================
set_config_cron('cron_last_run', date('Y-m-d H:i:s'));
