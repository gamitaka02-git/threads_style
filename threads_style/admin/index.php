...
try {
    ...
    if ($row) $latest_follower = (int)$row['follower_count'];
        ...
        <section id="section-dashboard" class="section active">
            ...
            <div class="kpi-grid">
                ...
                <div class="kpi-card">
                    ...
                    <div class="kpi-value <?= $token_days_left <= 7 ? 'style="color:var(--color-error)"' : ($token_days_left <= 14 ? 'style="color:var(--color-warning)"' : '') ?>"><?= $token_days_left ?>日</div>
                    <div class="kpi-change <?= $token_days_left <= 7 ? 'negative' : '' ?>"><?= $token_days_left <= 7 ? '要更新' : '有効' ?></div>
                    <div class="kpi-icon">🔑</div>
                </div>
                <?php endif; ?>
            </div>
            ...
            <div class="card" style="margin-bottom: var(--space-lg);">
                ...
                <div style="display:flex; align-items:center; gap: var(--space-lg); flex-wrap:wrap;">
                    <div>
                        <div style="font-size:var(--font-size-sm); color:var(--color-text-secondary); margin-bottom:4px;">最終実行確認</div>
                        <?php if ($cron_last_run): ?>
                            ...
                            <div class="text-xs text-muted" style="margin-top:4px;">
                                ...
                                if ($diff < 3600) echo round($diff / 60) . '分前';
                                elseif ($diff < 86400) echo round($diff / 3600) . '時間前';
                                else echo round($diff / 86400) . '日前';
                                ?>
                            </div>
                        <?php else: ?>
                            <div style="font-size:var(--font-size-md); color:var(--color-warning); font-weight:600;">未設定</div>
            ...
            <div class="card" style="margin-bottom: var(--space-lg);">
                ...
                <?php if ($next_scheduled): ?>
                    <div style="display:flex; align-items:center; gap: var(--space-lg); flex-wrap:wrap;">
                        <div>
                            <div style="font-size:var(--font-size-sm); color:var(--color-text-secondary); margin-bottom:4px;">投稿予定日時</div>
                            <div style="font-size:var(--font-size-lg); font-weight:700;"><?= htmlspecialchars($next_scheduled['scheduled_at']) ?></div>
                        </div>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    $badge_text = '下書き';
                                    switch ($post['status']) {
                                        case 'posted':    $badge_class = 'badge-success'; $badge_text = '投稿済'; break;
                                        case 'scheduled': $badge_class = 'badge-info';    $badge_text = '予約中'; break;
                                        case 'failed':    $badge_class = 'badge-error';   $badge_text = '失敗';   break;
                                    }
                                    ?>
                                ...
                                <td style="white-space:nowrap; color:var(--color-text-secondary);"><?= htmlspecialchars($post['created_at']) ?></td>
                            </tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
            ...
            <div class="tab-panel active" id="tab-post-create">
                <div class="post-composer">
                    <div class="post-composer-body">
                        <textarea id="postContent" placeholder="いま何を考えていますか？&#10;&#10;Threads に投稿する内容を入力してください..."></textarea>
                    </div>
                    ...
                    <div class="post-composer-footer">
                        <div class="post-composer-options">
                            ...
                            <div class="form-group" style="margin:0;">
                                <input type="datetime-local" id="scheduleAt" class="form-input" style="width:auto; padding:6px 10px; font-size:var(--font-size-xs);">
                            </div>
                        </div>
                        <div class="post-composer-actions">
                            ...
                            <button class="btn btn-primary btn-sm" onclick="publishPostNow()" style="background:var(--color-success);">今すぐ投稿</button>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="card">
                    ...
                    <div class="thread-posts" id="threadPosts">
                        <div class="thread-item">
                            <textarea class="form-textarea thread-textarea" placeholder="スレッド 1つ目の投稿..." rows="3"></textarea>
                        </div>
                        <div class="thread-item">
                            <textarea class="form-textarea thread-textarea" placeholder="スレッド 2つ目の投稿..." rows="3"></textarea>
                        </div>
                    </div>

                    <div class="flex justify-between items-center mt-lg" style="flex-wrap:wrap; gap:var(--space-sm);">
                        <div class="form-group" style="margin:0;">
                            <label class="form-label">予約日時</label>
                            <input type="datetime-local" id="threadScheduleAt" class="form-input" style="width:auto;">
                        </div>
                        <div class="flex gap-sm">
            ...
            <div class="tab-panel" id="tab-post-repurpose">
                <div class="card">
                    ...
                    <div class="form-group">
                        ...
                        <input type="url" id="repurposeUrl" class="form-input" placeholder="https://example.com/blog/my-article">
                ...
                <div class="card">
                    ...
                    <div id="analysisResult" class="hidden mt-lg">
                        <div class="card" style="background:var(--color-bg); border-color:var(--color-success);">
                            ...
                            <div id="analysisResultContent" class="text-sm" style="line-height:1.8; white-space:pre-wrap;"></div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="tab-panel" id="tab-ai-style">
                <div class="card">
                    ...
                    <div class="style-guide-editor">
                        <textarea id="styleGuideEditor" placeholder="# スタイルガイド&#10;&#10;ここにAI解析結果が表示されます。&#10;手動で編集することもできます。"></textarea>
            ...
            <div class="tab-panel" id="tab-ai-generate">
                <div class="card">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="generateTopic" class="form-input" placeholder="例: 朝のルーティン、プログラミング学習のコツ">
                    </div>

                    <div class="form-group">
                        <label class="form-label">追加の指示（任意）</label>
                        <textarea id="generateInstructions" class="form-textarea" rows="3" placeholder="例: カジュアルな口調で、絵文字多めに"></textarea>
                    </div>
            ...
            <div class="tab-panel" id="tab-analytics-keywords">
                <div class="card">
                    ...
                    <div class="flex gap-sm mb-lg">
                        <input type="text" id="newKeyword" class="form-input" placeholder="監視するキーワードを入力" style="flex:1;">
            ...
            <div class="settings-grid">
                ...
                <div class="settings-card">
                    ...
                    <div class="settings-card-body">
                        ...
                        <div class="token-countdown mt-md">
                            <div class="token-days <?= $token_days_left <= 7 ? 'danger' : ($token_days_left <= 14 ? 'warning' : 'safe') ?>">
                                <?= $token_days_left ?>
                            </div>
                            <div>
                ...
                <div class="settings-card">
                    ...
                    <div class="settings-card-body">
                        <div class="form-group">
                            ...
                            <input type="password" id="settingsGeminiKey" class="form-input" placeholder="Gemini API Key"
                                value="<?= htmlspecialchars(get_config('gemini_api_key')) ?>">
                            <p class="form-help"><a href="https://aistudio.google.com/" target="_blank">Google AI Studio</a> で取得</p>
                        </div>
                        <div class="form-group">
                            ...
                            <select id="settingsGeminiModel" class="form-select">
                                ...
                                <option value="gemini-3.1-flash-lite-preview" <?= $current_model === 'gemini-3.1-flash-lite-preview' ? 'selected' : '' ?>>Gemini 3.1 Flash Lite（最速・軽量）</option>
                                <option value="gemini-3-flash-preview" <?= $current_model === 'gemini-3-flash-preview' ? 'selected' : '' ?>>Gemini 3 Flash（バランス）</option>
                                <option value="gemini-2.5-flash-lite-preview-06-17" <?= $current_model === 'gemini-2.5-flash-lite-preview-06-17' ? 'selected' : '' ?>>Gemini 2.5 Flash Lite（安定）</option>
                            </select>
                        </div>
                ...
                <div class="settings-card">
                    ...
                    <div class="settings-card-body">
                        <div class="form-group">
                            ...
                            <input type="password" id="settingsLicenseKey" class="form-input" placeholder="XXXX-XXXX-XXXX-XXXX"
                ...
                <div class="settings-card">
                    ...
                    <div class="settings-card-body">
                        <div class="toggle-wrap mb-md">
                            <label class="toggle">
                                <input type="checkbox" id="settingsAutoPost" <?= get_config('auto_post_enabled') === '1' ? 'checked' : '' ?>>
                                <span class="toggle-slider"></span>
                            </label>
                        ...
                        <div class="toggle-wrap mb-md">
                            <label class="toggle">
                                <input type="checkbox" id="settingsAiLabelDefault" <?= get_config('ai_label_default') === '1' ? 'checked' : '' ?>>
                                <span class="toggle-slider"></span>
                            </label>
                ...
                <div class="settings-card">
                    ...
                    <div class="settings-card-body">
                        ...
                        <div style="background:var(--color-surface-alt); border-radius:var(--radius-md); padding:var(--space-md); margin-top:var(--space-md);">
                            <div class="flex items-center gap-md mb-sm">
                                <span style="font-size:18px;">ℹ️</span>
                                <strong class="text-sm">仕組み</strong>
                            </div>
                            <ul class="text-xs text-muted" style="list-style:disc; padding-left:var(--space-xl); line-height:2;">
                        ...
                        <?php if ($cron_last_run): ?>
                        <div class="flex items-center gap-md mt-md" style="padding:var(--space-md); background:var(--color-success-bg); border-radius:var(--radius-md);">
                            <span style="font-size:18px;">✅</span>
                            <div>
                                <div class="text-sm font-bold" style="color:var(--color-success);">Cronは正常に動作しています</div>
                                <div class="text-xs text-muted">最終実行: <?= htmlspecialchars($cron_last_run) ?></div>
                            </div>
                        </div>
                        <?php else: ?>
                        <div class="flex items-center gap-md mt-md" style="padding:var(--space-md); background:var(--color-warning-bg); border-radius:var(--radius-md);">
                            <span style="font-size:18px;">⚠️</span>
                            <div>
                                <div class="text-sm font-bold" style="color:var(--color-warning);">Cronがまだ設定されていません</div>
                ...
                <div class="settings-card">
                    ...
                    <div class="settings-card-body">
                        <p class="text-sm text-muted mb-md">現在のバージョン: <strong><?= htmlspecialchars($version) ?></strong></p>
                        <div class="flex gap-sm">
                            <button class="btn btn-secondary" onclick="checkForUpdate()" id="checkUpdateBtn">🔍 更新を確認</button>
                            <button class="btn btn-primary hidden" onclick="executeUpdate()" id="executeUpdateBtn">⬆️ アップデート実行</button>
                        </div>
